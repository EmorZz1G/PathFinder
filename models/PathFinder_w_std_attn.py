...
try:    
    ...
    from .attention import BGAttention, BGAttentionReduce, SelfAttention, CrossAttention
except:
    # from UNET import UNet2, UNets
    from attention import BGAttention, BGAttentionReduce, SelfAttention, CrossAttention
    ...
    def forward(self, x, emb, bs_mask):
        for layer in self:
            if isinstance(layer, BGAttentionReduce):
                ...
                raise NotImplementedError("BGAttentionReduce is not implemented in this version.")
            # elif isinstance(layer, GainAttention):
            #     x = layer(x, emb, ~bs_mask)
            elif isinstance(layer, UNET_CrossAttentionBlock):
                x = layer(x, emb)
            else:
                x = layer(x)
        return x
...
class UNET_CrossAttentionBlock(nn.Module):
    def __init__(self, in_channels, n_head: int=4, n_embd: int=128, d_context=32):
        super().__init__()
        
        self.groupnorm = nn.GroupNorm(32, in_channels, eps=1e-6)
        self.conv_input = nn.Conv2d(in_channels, n_embd, kernel_size=1, padding=0)
        ...
        self.layernorm_3 = nn.LayerNorm(n_embd)
        self.linear_geglu_1  = nn.Linear(n_embd, 3 * n_embd * 2)
        self.linear_geglu_2 = nn.Linear(3 * n_embd, n_embd)

        self.conv_output = nn.Conv2d(n_embd, in_channels, kernel_size=1, padding=0)
    ...
    def forward(self, x, context):
        ...
        n1, c1, h1, w1 = context.shape

        context = context.reshape((n1, c1, h1 * w1))
...
class PathFinder_w_std_attn(torch.nn.Module):
    
    def __init__(self, enc_dec_dims=[128,128]):
        super(PathFinder_w_std_attn, self).__init__()
    ...
    def forward(self, x):
        ...
        # build + tx
        # attention
        x = self.unet(build + tx, build, mask)
        x = self.gain_dec(x)
        return x
