...
def ConRuT(in_channels, out_channels, kernel, padding):
    return nn.Sequential(
        nn.ConvTranspose2d(
            in_channels, out_channels, kernel, stride=2, padding=padding
        ),
        nn.ReLU(inplace=False),
    )
...
class PMNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_up3 = ConRuT(512 + 512, 256, 3, 1)  # 上采样
    ...
    def forward(self, x):
        # Encoder
        # x (2,2,256,256)
        x1 = self.layer1(x)  # (2,64,65,65)
        x2 = self.layer2(x1)  # C=64 -> 256 -> 64 -> 64 -> 256
        x3 = self.reduce(x2)  # (2,256,65,65)
        x4 = self.layer3(x3)  # (2,512,33,33)
        x5 = self.layer4(x4)  # (2,512,33,33)
        x6 = self.layer5(x5)  # (2,512,33,33)
        x7 = self.aspp(x6)  # (2,1280,33,33) ， 多尺度卷积
        x8 = self.fc1(x7)  # (2,512,33,33)

        # Decoder
        xup5 = self.conv_up5(x8)
        xup5 = torch.cat([xup5, x5], dim=1)  # (2,1024,33,33)
        ...
        xup0 = self.conv_up0(xup1)  # (2,128,65,65)

        xup0 = F.interpolate(
            xup0, size=x.shape[2:], mode="bilinear", align_corners=False
        )
...
if __name__ == "__main__":
    # x = torch.randint(0,255,size=(1, 2, 256, 256)).float()
